...
class QuantumGravity2Strategy(BaseStrategy):
    ...
    def get_pinbar_search_zone(self, level: float, direction: str) -> dict:
        if direction == "long":
            # Сигнал LONG (підтримка) → чекаємо ПЕРЕКУПЛЕНІСТЬ → відкриваємо SHORT
            return {
                ...
                "entry_on": "low",  # Вхід на LOW свічки
                "stop_on": "high",
                "invalidate_on": "high"
            }
        else:
            # Сигнал SHORT (опір) → чекаємо ПЕРЕПРОДАНІСТЬ → відкриваємо LONG
            return {
                "search_after_consolidation": True,
                "wait_for_rsi": True,
                ...
                "entry_on": "high",  # Вхід на HIGH свічки
                "stop_on": "low",
                "invalidate_on": "low"
            }

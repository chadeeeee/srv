...
from utils.settings_manager import get_settings, get_bybit_base_url
import aiohttp
...
class RiskManager:
    def __init__(self):
        ...
        self.position_size_percent = settings.get(
            'position_size_percent') or CONFIG.get('POSITION_SIZE_PERCENT', 1.5)
        self.default_order_qty = settings.get(
            'default_order_qty') or CONFIG.get('DEFAULT_ORDER_QTY', 10)
        self.time_offset = 0
                ...
                async with session.get(url, timeout=5) as resp:
                    if resp.status == 200:
                        ...
                        if data and data.get("retCode") == 0:
                            ...
                            logger.debug(
                                f"Time offset updated: {self.time_offset}")
                            return self.time_offset
        except Exception:
            logger.debug(
                "RiskManager.sync_time: could not sync time, using local time")
    ...
    async def get_wallet_balance(self):
        try:
            ...
            try:
                    ...
                    async with session.get(url, params={"accountType": "UNIFIED"}, timeout=5) as resp:
                        if resp.status == 200:
                            ...
                            if data and data.get("retCode") == 0:
                                ...
                                logger.debug("Fetched wallet balance (detailed available at DEBUG)", extra={
                                             "show_console": True})
                                out["_SUMMARY"] = {
                ...
                logger.debug(
                    "Wallet API call failed; using local defaults", exc_info=True)
            wallet_balance = float(settings.get(
                "DEFAULT_ORDER_QTY", CONFIG.get("DEFAULT_ORDER_QTY", 10)))
            return {
                "_SUMMARY": {
    ...
    async def set_stop_loss(self, order_id, pair, direction, stop_price):
        try:
            logger.info(
                f"Setting SL for {pair}: {stop_price} (order {order_id})")
            return True
        except Exception as e:
    ...
    async def set_take_profit(self, order_id, pair, direction, take_profit_price):
        try:
            logger.info(
                f"Setting TP for {pair}: {take_profit_price} (order {order_id})")
            return True
        except Exception as e:
    ...
    async def display_balance(self):
        try:
            ...
            total_available = float(summary.get(
                "totalAvailableBalance", 0) or 0)
            now = time.time()

                    ...
                    async with session.get(url, params={"accountType": "UNIFIED"}, timeout=5) as resp:
                        if resp.status == 200:
                            ...
                            if data and data.get("retCode") == 0:
                                ...
                                logger.debug("Fetched wallet balance (detailed available at DEBUG)", extra={"show_console": True})
                                out["_SUMMARY"] = {
    ...
    async def display_balance(self):
        try:
            ...
            if now - self._last_balance_log_time >= self._balance_log_interval:
                ...
                logger.debug(f"Balance summary (suppressed INFO): Equity={total_equity:.2f} Available={total_available:.2f}")
        except Exception:

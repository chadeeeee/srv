...
class SignalHandler:
    _STOP_CREATE_TYPES = {"CreateByStopLoss", "CreateByStopOrder",
                          "CreateByTrailingStop", "CreateByStopLossSwitchMode"}
    _TAKE_PROFIT_CREATE_TYPES = {
    ...
    def __init__(self):
        ...
        self._balance_log_interval = float(
            CONFIG.get('BALANCE_LOG_INTERVAL', 300))
    ...
    async def _get_max_retries(self):
        ...
        if not self._db_pool:
            logger.error(
                "База данных не подключена, используется значение max_retries=1")
            return 1
        try:
            ...
            logger.error(
                f"Не удалось получить max_retries из базы данных: {e}, используется значение по умолчанию=1")
            return 1

    async def _get_rsi_settings_db(self):
        await self._init_db()
        defaults = {"rsi_low": 30, "rsi_high": 70,
                    "rsi_period": 14, "rsi_interval": "1"}
        if not self._db_pool:
            logger.warning(
                "БД недоступна, використовуються RSI настройки по умолчанию")
            return defaults
        try:
            async with self._db_pool.acquire() as conn:
    ...
    async def get_server_time_offset(self):
        ...
        logger.warning(
            "Не удалось получить время сервера, используется локальное время")
        return 0
    ...
    async def _monitor_limit_order_lifetime(self, pair, order_id, check_interval=10):
        ...
        try:
            ...
            if lifetime_val is None:
                logger.info(
                    f"Моніторинг часу життя лімітки вимкнено для {pair} (налаштування вимкнено)")
                return
            ...
            if lifetime_minutes <= 0:
                logger.info(
                    f"Моніторинг часу життя лімітки вимкнено для {pair}")
                return
            ...
            if not meta:
                logger.warning(
                    f"Немає метаданих для ордера {order_id}, пропускаємо моніторинг часу життя")
                return
            ...
            while True:
                ...
                if not current_meta:
                    logger.info(
                        f"Ордер {order_id} для {pair} більше не існує в метаданих")
                    return
                ...
                try:
                    ...
                    if position and float(position.get("size", 0) or 0) > 0:
                        logger.info(
                            f"Лімітний ордер {order_id} для {pair} виконався - позиція відкрита")
                        return
                except Exception as e:
                ...
                if elapsed >= lifetime_seconds:
                    ...
                    try:
                        ...
                        if cancel_result:
                            logger.info(
                                f"Лімітний ордер {order_id} для {pair} успішно скасовано")
                            try:
                                await notify_user(
                            ...
                            logger.error(
                                f"Не вдалося скасувати лімітний ордер {order_id} для {pair}")
                    except Exception as e:
                        logger.error(
                            f"Помилка скасування ордера {order_id} для {pair}: {e}")
            ...
            logger.error(
                f"Помилка моніторингу часу життя лімітки для {pair}: {e}")
            return
    ...
    async def _cancel_order(self, pair, order_id):
        ...
        try:
            ...
            if result and result.get("retCode") == 0:
                ...
                ret_msg = result.get(
                    "retMsg", "Невідома помилка") if result else "Немає відповіді"
                logger.error(
                    f"Не вдалося скасувати ордер {order_id} для {pair}: {ret_msg} (код {ret_code})")
                return False
    ...
    async def _watch_position_exit(self, pair, poll_interval=3):
        ...
        if not order_id:
            logger.warning(
                f"Нет order_id для пары {pair}. Попытка получить позицию напрямую.")
            position = await self._take_profit_helper.get_position(pair)
            if position and float(position.get("size", 0) or 0) > 0:
                synthetic_id = f"synthetic_{pair}_{time.time()}"
                logger.info(
                    f"Создан синтетический order_id {synthetic_id} для мониторинга позиции {pair}")
                ...
                logger.warning(
                    f"Не найдена позиция для {pair}, прекращение мониторинга")
                return
        ...
        while True:
            ...
            if not meta:
                logger.error(
                    f"Нет метаданных для order_id {order_id}, пара {pair}. Попытка восстановить...")
                if pair in self._pair_to_order:
                    current_order_id = self._pair_to_order[pair]
                    if current_order_id != order_id and current_order_id in self._position_meta:
                        logger.warning(
                            f"Найдено расхождение order_id для {pair}. Используем актуальные метаданные.")
                        order_id = current_order_id
                        meta = self._position_meta.get(order_id)
                if not meta:
                    logger.error(
                        f"Невозможно восстановить метаданные для {pair}, прекращение мониторинга")
                    return
            ...
            try:
                ...
                logger.error(
                    f"Не удалось получить данные позиции для {pair}: {exc}")
                ...
                if size > 0:
                    ...
                    if not meta["sl_set"] and not sl_attempted and meta.get("stop_loss"):
                        ...
                        try:
                            ...
                            logger.info(
                                f"НЕГАЙНЕ встановлення SL для {pair}...")
                            logger.info(
                                f"   Позиція виявлена: entry={entry_price:.8f}")
                            logger.info(
                                f"   Встановлюємо SL: {stop_loss_price:.8f}")
                            ...
                            for sl_attempt in range(MAX_SL_ATTEMPTS):
                                ...
                                if success:
                                    logger.info(
                                        f"SL встановлено ОДРАЗУ: {stop_loss_price:.8f}")
                                    meta["sl_set"] = True
                                    sl_success = True
                                    break
                                else:
                            ...
                            if not sl_success:
                                logger.error(
                                    f"Не вдалося встановити SL після {MAX_SL_ATTEMPTS} спроб")
                    ...
                    if not meta["open_notified"]:
                        try:
                            ...
                            direction_emoji = "🟢" if meta.get(
                                'direction') == 'long' else "🔴"
                            direction_text = "LONG" if meta.get(
                            ...
                            logger.error(
                                f"Помилка відправки повідомлення про відкриття: {e}")
                    ...
                    if not meta["rsi_tracker_started"]:
                        ...
                        rsi_low = float(
                            rsi_low_val) if rsi_low_val is not None else None
                        rsi_high = float(
                        ...
                        if should_start_tracker:
                            ...
                            asyncio.create_task(
                                self._take_profit_helper.track_position_rsi(
                                    ...
                                    poll_seconds=max(
                                        15, int(self._interval_to_seconds(rsi_interval) // 2))
                                )
                            )
                            meta["rsi_tracker_started"] = True
                        else:
                            logger.info(
                                f"RSI TP трекер НЕ запущено для {pair} (налаштування вимкнено для {direction})")
                            meta["rsi_tracker_started"] = True
            except Exception as exc:
                logger.error(
                    f"Не удалось запустить RSI трекер для {pair}: {exc}")
            if size <= 0:
                try:
                    direction = meta.get("direction")
                    meta.get("stop_loss")
                    ...
                    logger.error(
                        f"Не удалось класифіковане закриття для {pair}: {e}")
                    meta["exit_reason"] = meta.get(
    ...
    async def _get_latest_close_order(self, pair):
        ...
        for endpoint in endpoints:
            ...
            if not orders:
                continue
            orders.sort(key=lambda o: int(o.get("updatedTime")
                        or o.get("createdTime") or 0), reverse=True)
            for order in orders:
    ...
    async def _handle_position_exit(self, pair, reason, meta, context=None):
        ...
        if position_key and position_key in self._managed_external_positions:
            ...
            logger.info(
                f"Удалено {position_key} из управляемых внешних позиций")
        ...
        logger.info(
            f"   - Цена входа: {entry_price:.6f}" if entry_price else "   - Цена входа: N/A")
        logger.info(
            f"   - Цена закрытия: {closing_price:.6f}" if closing_price else "   - Цена закрытия: N/A")
        logger.info(
            f"   - Стоп-лосс: {stop_loss:.6f}" if stop_loss else "   - Стоп-лосс: N/A")
        logger.info(
            f"   - Тейк-профит: {take_profit:.6f}" if take_profit else "   - Тейк-профіт: N/A")
        ...
        if reason == "take_profit" and context:
            ...
            if rsi_value is not None and rsi_threshold is not None:
                logger.info(
                    f"   - RSI при закритті: {rsi_value:.2f} (поріг: {rsi_threshold:.2f})")
                ...
                try:
                    ...
                    if entry_price and closing_price:
                        if direction == "long":
                            pnl_percent = (
                                (closing_price - entry_price) / entry_price * 100)
                        else:
                            pnl_percent = (
                                (entry_price - closing_price) / entry_price * 100)
            ...
            try:
                ...
                logger.error(
                    f"Помилка відправки TG повідомлення про закриття: {e}")

        logger.info(f"   - Попытки: {retry_count}/{max_retries}")

        if source and source.startswith("external"):
            logger.info(
                f"Позиция {pair} была открыта ВРУЧНУЮ (source={source})")
        ...
        if reason in {"position_closed", "tracking_stopped"}:
            try:
                ...
                meta["exit_reason"] = reason
                logger.info(
                    f"Переклассифицирована причина для {pair}: {old_reason} → {reason}")
            except Exception as exc:
                logger.error(
                    f"Не удалось переклассифицировать причину закрытия для {pair}: {exc}")
                reason = "manual_close"

        if reason == "take_profit":
            logger.info(
                f"Позиция по {pair} ({direction}) закрыта по тейк-профиту")
            ...
            if closing_price and stop_loss:
                ...
                position_closed_by_stop(
                    pair, direction, closing_price, stop_loss, distance)
            else:
                logger.warning(
                    f"Позиція по {pair} ({direction}) закрита по стоп-лоссу")
            ...
            if closing_price and stop_loss:
                ...
                logger.info(
                    f"Позиція по {pair} ({direction}) була закрита вручну")
        else:
            logger.info(
                f"Позиція по {pair} закрита з невідомою причиною: {reason}")

        if reason == "stop_loss" and retry_count < max_retries:
            from utils.logger import reentry_allowed
            reentry_allowed(
                pair, f"Сработал стоп, попытка {retry_count + 1}/{max_retries}")
            ...
            if CONFIG.get("IGNORE_PINBAR", False):
                logger.info(
                    f"Режим IGNORE_PINBAR: немедлений повторний вхід для {pair}")
                try:
                    meta["retry_count"] = retry_count + 1
                    success = await self._reenter_immediately(pair, direction, meta)
                    if success:
                        logger.info(
                            f"Повторний вхід успішно виконано для {pair}")
                    else:
                        logger.error(f"Повторний вхід не удалася для {pair}")
                except Exception as exc:
                    logger.error(
                        f"Немедлений повторний вхід не удалася для {pair}: {exc}")
            else:
                signal_level = meta.get("signal_level") or entry_price
                if signal_level:
                    try:
                        logger.info(
                            f"Перезапуск пошуку пин-бара для {pair} около уровня {signal_level}")
                        asyncio.create_task(self._start_monitor_pinbar_task(
                            pair, signal_level, direction))
                    except Exception as exc:
                        logger.error(
                            f"Не вдалося перезапустити пошук пин-бара для {pair}: {exc}")
                else:
                    logger.info(
                        "Нет записанного уровня для повторного входа по пин-бару")
        else:
            from utils.logger import reentry_blocked
            if reason == "stop_loss":
                reentry_blocked(
                    pair, f"Достигнут максимум спроб ({retry_count}/{max_retries})")
                logger.warning(
                    f"Достигнут максимум спроб для {pair}: {retry_count}/{max_retries}")
            else:
                ...
                logger.info(
                    f"   - Кількість спроб: {retry_count}/{max_retries}")
        ...
        if reason == "manual_close":
            ...
            logger.info(
                f"👤 Вручну закрита позиція {pair} відслідковується для запобігання переоткриття")
        meta["finalized"] = True
        order_id = self._pair_to_order.pop(pair, None)
        if order_id:
            self._position_meta.pop(order_id, None)
    ...
    async def _signed_request(self, method, endpoint, params=None):
        ...
        signature = hmac.new(self.api_secret.encode(
            'utf-8'), pre_sign.encode('utf-8'), hashlib.sha256).hexdigest()
        headers = {
            ...
            try:
                ...
                if not text.strip():
                    ...
                    if attempt == max_retries:
                        logger.error(
                            f"Порожня відповідь {method} {endpoint} після {attempt} спроб")
                        return None
                ...
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(
                            f"Невалідний JSON {method} {endpoint}: {text[:120]}")
                        return None
                    await asyncio.sleep(base_delay * attempt)
                    continue
                ...
                if resp.status != 200 and attempt == max_retries:
                    logger.error(
                        f"HTTP {resp.status} {method} {endpoint}: {data}")
                return data
            except aiohttp.ClientConnectionError as e:
                logger.warning(
                    f"Connection error {method} {endpoint} (attempt {attempt}/{max_retries}): {e}")
                ...
                logger.warning(
                    f"Client error {method} {endpoint} (attempt {attempt}/{max_retries}): {e}")
                if attempt == max_retries:
                ...
                logger.warning(
                    f"Unexpected error {method} {endpoint} (attempt {attempt}/{max_retries}): {e}")
                if attempt == max_retries:
                    logger.error(
                        f"Непередбачена помилка {method} {endpoint}: {e}")
                    return None
                await asyncio.sleep(base_delay * attempt)
        return None
            ...
            for coin in coins:
                if coin.get("coin") == "USDT":
                    ...
                    wallet_balance = float(
                        usdt_info.get("walletBalance", 0) or 0)
                    available = wallet_balance - \
                        float(usdt_info.get("totalOrderIM", 0) or 0)
                    now = time.time()
                    if now - self._last_balance_log_time >= self._balance_log_interval:
                        ...
                        logger.info(
                            f"   - В ордерах: {float(usdt_info.get('totalOrderIM', 0)):.2f} USDT")
    ...
    async def _get_symbol_specs(self, symbol):
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.get(url) as response:
                ...
                if not text:
                    raise ValueError(
                        f"Пустая інформація об інструмент для {symbol}")
                data = json.loads(text)
                if data.get("retCode") != 0:
                    raise ValueError(
                        f"Ошибка інформації об інструмент для {symbol}: {data}")
                items = data.get("result", {}).get("list", [])
                if not items:
                    raise ValueError(
                        f"Нет інформації об інструмент для {symbol}")
    ...
    async def _set_leverage(self, symbol, leverage):
        ...
        leverage = min(
            max(leverage, specs["min_leverage"]), specs["max_leverage"])
        params = {
            "category": "linear",
            "symbol": symbol,
        ...
        if not result or result.get("retCode") != 0:
            logger.warning(
                f"Не вдалося встановити кредитне плече {leverage:.2f} для {symbol}: {result}")
    ...
    async def _calculate_extremum_stop_loss(self, symbol, direction):
        try:
            ...
            try:
                stop_behind_offset = CONFIG.get(
                    "STOP_BEHIND_EXTREMUM_OFFSET", 0.002)
            except:
            ...
            logger.error(
                f"_calculate_extremum_stop_loss error for {symbol}: {e}")
            return None
    ...
    async def _is_drawdown_blocked(self):
        ...
        if is_drawdown_protection_active():
            logger.warning(
                "Drawdown protection active: new orders disabled. Use START to re-enable.")
            return True
        try:
    ...
    async def place_limit_order(self, pair, direction, price, quantity_usdt, stop_loss=None, take_profit=None, reduce_only=False, metadata=None):
        if await self._is_drawdown_blocked():
            logger.warning(
                f"Пропускаем размещення лимитного ордера для {pair}: защита по просадке")
            return None
        ...
        if max_trades is not None:
            ...
            try:
                ...
                if result and result.get("retCode") == 0:
                    ...
                    conditional_count = len([o for o in active_orders if o.get(
                        "orderStatus") in ["New", "PartiallyFilled", "Untriggered"]])
                else:
                    conditional_count = 0
            except Exception:
                conditional_count = 0
            ...
            if total_active >= max_trades:
                ...
                logger.warning(
                    f"   Відкрито: {total_active} (позицій: {active_count} + ордерів: {conditional_count})")
        ...
        if size > 0:
            logger.info(
                f"Позиція по {pair} вже відкрита (розмір {size}), пропускаємо відкриття нової.")
            return None
        ...
        if stop_loss_float is not None:
            ...
            stop_loss_float = self._quantize_price(
                stop_loss_float, price_step, sl_rounding)
            if direction == "long" and stop_loss_float >= price:
                stop_loss_float = self._quantize_price(
                    price * 0.993, price_step, ROUND_DOWN)
                logger.warning(
                    f"Скорректирован SL после квантования: {stop_loss_float}")
            elif direction == "short" and stop_loss_float <= price:
                stop_loss_float = self._quantize_price(
                    price * 1.007, price_step, ROUND_UP)
                logger.warning(
                    f"Скорректирован SL после квантования: {stop_loss_float}")
        if take_profit_float is not None:
            tp_rounding = ROUND_UP if direction == "long" else ROUND_DOWN
            take_profit_float = self._quantize_price(
                take_profit_float, price_step, tp_rounding)
        sl_percent = None
        tp_percent = None
        ...
        if take_profit_float and price:
            tp_percent = (take_profit_float - price) / price * \
                100 if direction == "long" else (
                    price - take_profit_float) / price * 100
        quantity_usdt_value = float(quantity_usdt)
        margin_usdt = quantity_usdt_value
        leverage = 10
        notional_usdt = margin_usdt * leverage
        quantity_asset = self._quantize(
            notional_usdt / price, specs["qty_step"])
        ...
        if result and result.get("retCode") == 0:
            limit_order_placed(direction, pair, price_str,
                               f"{order_value:.2f} USDT @ {leverage}x (маржа {margin_usdt:.2f})")

            if stop_loss_float is not None:
                logger.info(
                    f"SL буде встановлений після відкриття: {stop_loss_float:.8f} ({sl_percent:.2f}%)")
            if take_profit_float is not None:
                logger.info(
                    f"TP буде встановлений після відкриття: {take_profit_float:.8f} ({tp_percent:.2f}%)")
            ...
            if order_id:
                ...
                asyncio.create_task(
                    self._monitor_limit_order_lifetime(pair, order_id))
            return result["result"]
        else:
            if not result:
                logger.error(
                    f"Не удалось разместить лимитний ордер для {pair}: Нет відповіді від API")
            elif result.get("retCode") != 0:
                ret_code = result.get("retCode")
                ret_msg = result.get("retMsg", "Неизвестная ошибка")
                logger.error(
                    f"Не вдалося розмістити лімітний ордер для {pair}: {ret_msg} (код {ret_code})")
            else:
                logger.error(
                    f"Не вдалося розмістити лімітний ордер для {pair}: Невідомий формат відповіді - {result}")
        return None
    ...
    async def place_trigger_order(self, pair, direction, trigger_price, quantity_usdt, stop_loss=None, take_profit=None, metadata=None):
        ...
        if await self._is_drawdown_blocked():
            logger.warning(
                f"Пропускаємо trigger order для {pair}: захист по просадці")
            return None
        ...
        trigger_price_quantized = self._quantize_price(
            trigger_price_float, price_step, ROUND_DOWN)

        if trigger_price_quantized < current_price:

            trigger_direction = 2
            logger.info(
                f"📉 Trigger НИЖЧЕ поточної: спрацює при ПАДІННІ до {trigger_price_quantized:.8f}")
        else:

            trigger_direction = 1
            logger.info(
                f"📈 Trigger ВИЩЕ поточної: спрацює при ЗРОСТАННІ до {trigger_price_quantized:.8f}")

        logger.info(
            f"   � {direction.upper()}: поточна {current_price:.8f} → trigger {trigger_price_quantized:.8f}")
        ...
        notional_usdt = margin_usdt * leverage
        quantity_asset = self._quantize(
            notional_usdt / trigger_price_quantized, specs["qty_step"])
        if quantity_asset < specs["min_qty"]:
            quantity_asset = specs["min_qty"]
        ...
        logger.info(
            f"   Розмір: {quantity_asset} ({order_value:.2f} USDT @ {leverage}x)")
        if stop_loss_float:
        ...
        if result and result.get("retCode") == 0:
            logger.info(
                f"✅ TRIGGER ORDER розміщено: {pair} {direction.upper()}")
            ...
            if not result:
                logger.error(
                    f"❌ Не вдалося розмістити trigger order для {pair}: Немає відповіді від API")
            elif result.get("retCode") != 0:
                ret_code = result.get("retCode")
                ret_msg = result.get("retMsg", "Невідома помилка")
                logger.error(
                    f"❌ Не вдалося розмістити trigger order для {pair}: {ret_msg} (код {ret_code})")
            else:
                logger.error(
                    f"❌ Не вдалося розмістити trigger order для {pair}: Невідомий формат відповіді")
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if not result or result.get("retCode") != 0:
                        logger.warning(
                            f"Не вдалося отримати статус trigger order {order_id}")
                        continue
                    ...
                    if not orders:
                        ...
                        logger.info(
                            f"🔍 Trigger order {order_id} не знайдено в активних - перевіряємо позицію...")

                        position = await self._take_profit_helper.get_position(pair)
                        if position and float(position.get("size", 0) or 0) > 0:
                            logger.info(
                                f"✅ Trigger order {pair} АКТИВОВАНО - позиція відкрита!")
                            ...
                            logger.info(
                                f"   Фактична ціна входу: {actual_entry:.8f}")

                            stop_loss = meta.get("stop_loss")
                            if stop_loss:
                                logger.info(
                                    f"🛡 Встановлення SL: {stop_loss:.8f}")
                                for attempt in range(MAX_SL_ATTEMPTS):
                                    ...
                                    if success:
                                        meta["sl_set"] = True
                                        logger.info(
                                            f"✅ SL встановлено після активації trigger")
                                        break
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Помилка відправки повідомлення: {e}")
                            ...
                            try:
                                ...
                                async def _on_rsi_exit(sym, dirn, reason, context):
                                    logger.info(
                                        f"RSI TP для trigger order {sym}: {reason}")
                                ...
                                logger.error(
                                    f"Помилка запуску RSI трекера: {e}")

                            asyncio.create_task(
                                self._watch_position_exit(pair))
                            return
                        else:

                            logger.warning(
                                f"⚠ Trigger order {order_id} для {pair} не активовано - статус невідомий")
                            meta["finalized"] = True
                            return
                    else:
                        order = orders[0]
                        ...
                        logger.debug(
                            f"Trigger order {pair} статус: {order_status}")
                        ...
                        if order_status in ["New", "Untriggered"]:
                            ...
                            logger.info(
                                f"✅ Trigger order {pair} виконано зі статусом {order_status}")

                            position = await self._take_profit_helper.get_position(pair)
                            if position and float(position.get("size", 0) or 0) > 0:
                                logger.info(
                                    f"✅ Позиція по {pair} підтверджена!")
                                ...
                                try:
                                    ...
                                    rsi_low = float(rsi_cfg.get("rsi_low", 30))
                                    rsi_high = float(
                                        rsi_cfg.get("rsi_high", 70))
                                    rsi_period = int(
                                        rsi_cfg.get("rsi_period", 14))
                                    rsi_interval = '1'
                                ...
                                logger.warning(
                                    f"⚠ Trigger order {pair} Filled, але позиція не знайдена. Чекаємо...")
                                await asyncio.sleep(1)
                                continue
                        elif order_status in ["Cancelled", "Rejected"]:
                            logger.warning(
                                f"⚠ Trigger order {pair} скасовано зі статусом {order_status}")
                            meta["finalized"] = True
                            return

                except Exception as e:
                    logger.error(
                        f"Помилка моніторингу trigger order {pair}: {e}")
                    await asyncio.sleep(poll_interval)
            ...
            logger.error(
                f"Критична помилка моніторингу trigger order {pair}: {e}")
            import traceback
            logger.error(traceback.format_exc())
        finally:
            self._active_monitors -= 1
    ...
    async def _wait_and_place_limit(self, pair, direction, target_price, quantity_usdt, stop_loss_price, take_profit_price, metadata, timeout_minutes=60, poll_interval=1.0):
        try:
            ...
            while time.time() <= deadline:
                ...
                if (direction == "long" and cur <= float(target_price)) or (direction == "short" and cur >= float(target_price)):
                    try:
                        ...
                        entry_price = self._quantize_price(
                            float(target_price), price_step, rounding)
                        logger.info(
                            f"Target reached for {pair} (cur={cur:.8f}) — placing limit @ {entry_price}")
                        ...
                        if result:
                            logger.info(
                                f"Placed delayed limit for {pair} @ {entry_price}")
                        else:
                            logger.error(
                                f"Failed to place delayed limit for {pair} @ {entry_price}")
                        return result
                    except Exception as e:
                        logger.error(
                            f"Error placing delayed limit for {pair}: {e}")
                        return None
                await asyncio.sleep(poll_interval)
            logger.warning(
            ...
            logger.error(
                f"Unexpected error in _wait_and_place_limit for {pair}: {e}")
            return None

    async def place_signal_limit_order(self, pair, direction, entry_price, channel_id=None):
        if await self._is_drawdown_blocked():
            logger.warning(
                f"Пропускаем сигнальний лимит для {pair}: защита по просадке")
            return None
        direction = (direction or "").lower()
        if direction not in ("long", "short"):
            logger.error(
                f"Неподдерживаемая сторона '{direction}' для сигнала на {pair}")
            return None
        ...
        if pair in self._manually_closed_positions:
            ...
            logger.info(
                f"Позиція {pair} була закрита вручну, але буде знову відкрита на основі нового сигналу")
        ...
        try:
            ...
            if "symbol invalid" in error_msg.lower() or "retCode': 10001" in error_msg:
                logger.error(
                    f"❌ Символ {pair} НЕ ІСНУЄ на Bybit! Пропускаю сигнал.")
                ...
                logger.error(
                    f"❌ Помилка отримання специфікацій для {pair}: {e}")
            return None
        ...
        logger.info(
            f"🔍 Запуск моніторингу для {pair} (Strategy: {strategy.name})")
        ...
        try:
            ...
            logger.error(
                f"Не вдалося запустити задачу monitor_pinbar для {pair}: {e}")
            return None
    ...
    async def verify_limit_order_capability(self, symbol: str) -> bool:
        try:
            ...
            logger.error(
                f"verify_limit_order_capability error for {symbol}: {exc}")
            return False
    ...
    async def start_external_trade_monitor(self):
        ...
        self._external_monitor_task = asyncio.create_task(
            self._external_trade_monitor_loop())
    ...
    async def stop_external_trade_monitor(self):
        ...
        if task:
            ...
            try:
                ...
                logger.warning(
                    f"Error awaiting external monitor task cancellation: {e}")
    ...
    async def _check_positions_via_pybit(self):
        try:
            ...
            if not session:
                logger.warning(
                    "PyBit session not available, falling back to API")
                return []
            ...
            try:
                ...
                if "Retryable error" in error_msg or "network" in error_msg.lower():
                    logger.warning(
                        f"PyBit network error, recreating session: {e}")
                    self._pybit_session = None
                    return []
                else:
            ...
            if result.get("retCode") != 0:
                logger.error(
                    f"pybit error getting positions: {result.get('retMsg')}")
                return []
        ...
        try:
            ...
            while getattr(self, "_external_trade_monitor_running", False):
                try:
                    ...
                    api_positions = []
                    if result and result.get("retCode") == 0:
                        api_positions = result.get(
                            "result", {}).get("list", [])

                    positions_to_process = pybit_positions if pybit_positions else api_positions
                    active_positions = [
                        p for p in positions_to_process if float(p.get("size", 0)) > 0]
                    ...
                    for closed_key in closed_keys:
                        logger.info(
                            f"Внешняя позиция {closed_key} закрита - удаляем из управляемих")
                    ...
                    for pos in active_positions:
                        ...
                        direction = "long" if pos.get(
                            "side") == "Buy" else "short"
                        entry_price = float(pos.get("avgPrice", 0) or 0)

                        logger.info(
                            f"===== ОБНАРУЖЕНА НОВАЯ РУЧНАЯ ПОЗИЦИЯ =====")
                        ...
                        try:
                            ...
                            logger.info(
                                f"Получаю свечу для {pair} на TF {tf_display} (API: {tf_api})")
                            ...
                            if candles and len(candles) >= 2:
                                ...
                                candle_high_low_info(
                                    pair, tf_display, c_high, c_low)
                            else:
                                logger.warning(
                                    f"Нет данных свечей для {pair} на TF {tf_display}")
                        except Exception as e:
                            logger.error(
                                f"Ошибка получения High/Low свечи для {pair}: {e}")
                            import traceback
                            logger.error(traceback.format_exc())

                        from utils.logger import external_position_detect
                        external_position_detect(
                            pair, direction, entry_price, pos.get('leverage', 'N/A'))
                        ...
                        try:
                            ...
                            if stop_loss_str and stop_loss_str != '' and stop_loss_str != '0':
                                has_sl = True
                                logger.info(
                                    f"Позиція вже має SL: {stop_loss_str}")
                        except Exception:
                            has_sl = False
                        ...
                        if not has_sl and entry_price > 0:
                            try:
                                ...
                                if extremum_data:
                                    ...
                                    from utils.logger import candle_high_low_info
                                    candle_high_low_info(
                                        pair, tf_display, candle_high, candle_low)
                                    ...
                                    if direction == "long" and stop_price >= entry_price:
                                        ...
                                        logger.warning(
                                            f"Використовую резервний SL: {stop_price:.8f}")
                                    ...
                                    logger.info(
                                        f"Встановлюю SL для {pair} на екстремумі свічки...")
                                    ...
                                    if success:
                                        ...
                                        logger.info(
                                            f"✅ SL ВСТАНОВЛЕНО НА ЕКСТРЕМУМІ СВІЧКИ:")
                                        logger.info(
                                            f"   Свічка (TF {tf_display}): H={candle_high:.8f} L={candle_low:.8f}")
                                        logger.info(
                                            f"   Екстремум ({direction.upper()}): {extremum_value:.8f}")
                                        logger.info(
                                            f"   Стоп: {stop_price:.8f}")

                                        self._managed_external_positions.add(
                                            pos_key)
                                        ...
                                        try:
                                            ...
                                            logger.error(
                                                f"Помилка відправки Telegram повідомлення про SL: {e}")
                                    else:
                                        logger.error(
                                            f"Не вдалося встановити SL для {pair}")
                            except Exception as e:
                                logger.error(
                                    f"Не удалось встановити SL для {pair}: {e}")
                                import traceback
                                logger.error(traceback.format_exc())
                        elif has_sl:
                        ...
                        try:
                            ...
                            rsi_settings_from_db(
                                pair, rsi_exit_short, rsi_exit_long, rsi_length, rsi_interval)

                            logger.info(f"RSI настройки з БД для {pair}:")
                            logger.info(
                                f"   - RSI Low (SHORT exit): {rsi_exit_short}")
                            logger.info(
                                f"   - RSI High (LONG exit): {rsi_exit_long}")
                            ...
                            if direction == "long":
                                logger.info(
                                    f"   - Поріг закриття (RSI High): {rsi_exit_long}")
                                logger.info(
                                    f"   - Закриється при RSI >= {rsi_exit_long}")
                                threshold_text = f"RSI >= {rsi_exit_long}"
                                distance = rsi_exit_long - current_rsi
                            else:
                                logger.info(
                                    f"   - Поріг закриття (RSI Low): {rsi_exit_short}")
                                logger.info(
                                    f"   - Закриється при RSI <= {rsi_exit_short}")
                            ...
                            rsi_interval_seconds = self._interval_to_seconds(
                                rsi_interval)
                            poll_seconds = max(
                                15, int(rsi_interval_seconds // 2))

                            logger.info(
                                f"   - RSI інтервал: {rsi_interval} ({rsi_interval_seconds}s)")
                            logger.info(
                                f"   - Опитування кожні: {poll_seconds}s)")
                            ...
                            logger.error(
                                f"Помилка запуску RSI трекера для {pair}: {e}")
                            import traceback
                            logger.error(traceback.format_exc())
    ...
    async def _start_monitor_pinbar_task(self, pair, level, direction, strategy=None):
        try:
            ...
            monitor = getattr(mod, "monitor_pinbar", None) or getattr(
                mod, "monitor_and_trade", None)
            if monitor is None:
                logger.error(
                    f"monitor_pinbar/monitor_and_trade not found in analysis.signals for {pair}")
                return

            if not strategy:
                logger.error(
                    f"КРИТИЧНА ПОМИЛКА: strategy не передана для {pair}!")
                from trading.strategies import QuantumPremium2Strategy
            ...
            logger.info(
                f"Запуск монітора з стратегією: {strategy.name} (channel: {strategy.channel_id})")
            ...
            if getattr(monitor, "__name__", "") == "monitor_and_trade":
                ...
                logger.info(
                    f"Strategy додана в settings: {strategy.name} (channel: {strategy.channel_id})")
            ...
            logger.error(
                f"Failed to start monitor_pinbar task for {pair}: {e}")
            import traceback
            logger.error(traceback.format_exc())
    ...
    async def get_real_time_price(self, pair):
        ...
        try:
            ...
            logger.error(
                f"Ошибка получения цены в реальном времени для {pair}: {e}")
            return None
    ...
    async def set_stop_loss(self, pair, price, direction):
        ...
        try:
            ...
            if result and result.get("retCode") == 0:
                ...
                return True
            logger.error(
                f" Не вдалося встановити стоп-лосс для {pair}: {result}")
            return False
        except Exception as e:
    ...
    async def set_take_profit(self, pair, price, direction):
        ...
        try:
            ...
            if result and result.get("retCode") == 0:
                ...
                return True
            logger.error(
                f"Не удалось установить тейк-профіт для {pair}: {result}")
            return False
        except Exception as e:
    ...
    async def _determine_exit_reason(self, pair, meta, closing_price=None):
        ...
        try:
            ...
            if not close_order:
                if meta.get("rsi_tracker_started"):
                    logger.info(
                        f" Нет close_order для {pair}, но RSI трекер был активен - это RSI take-profit")
                    return "take_profit"
            ...
            if order_type == "Market" and close_order.get("reduceOnly"):
                if meta.get("rsi_tracker_started"):
                    logger.info(
                        f" Market reduceOnly ордер + активный RSI трекер для {pair} = RSI take-profit")
                    return "take_profit"

            if create_type in self._USER_CLOSE_CREATE_TYPES:
                logger.info(
                    f"⚠️ Закриття НЕ по ціні стопа/ТП, визначено як MANUAL_CLOSE (createType={create_type})")
                return "manual_close"

            if meta.get("rsi_tracker_started"):
                logger.info(
                    f" Fallback: RSI трекер був активний для {pair} - це take-profit")
                return "take_profit"
    ...
    async def _reenter_immediately(self, pair, direction, meta):
        ...
        try:
            logger.info(
                f" Attempting immediate re-entry for {pair} {direction}")

            current_price = await self.get_real_time_price(pair)
            if not current_price:
                logger.error(
                    f"Cannot get current price for immediate re-entry on {pair}")
                return False
            ...
            if extremum_data:
                ...
                logger.info(
                    f" Re-entry SL calculated from extremum: {stop_loss_price:.8f}")
            else:

                logger.warning(
                    f" Failed to get extremum for re-entry, using fallback offset")
                if direction == "long":
                    stop_loss_price = self._quantize_price(
                        entry_price * (1 - stop_loss_offset), price_step, ROUND_DOWN)
                else:
                    stop_loss_price = self._quantize_price(
                        entry_price * (1 + stop_loss_offset), price_step, ROUND_UP)

            if direction == "long":
                entry_price = self._quantize_price(
                    current_price, price_step, ROUND_DOWN)
            else:
                entry_price = self._quantize_price(
                    current_price, price_step, ROUND_UP)
            ...
            if result:
                logger.info(
                    f" Immediate re-entry successful for {pair} @ {entry_price}")
                return True
            else:
    ...
    async def start_rsi_tracker(self, pair, direction):
        ...
        try:
            ...
            logger.info(
                f"🎯 Manual Start: RSI Tracker for {pair} ({direction})")
            logger.info(
                f"   Settings: High={rsi_high}, Low={rsi_low}, Len={rsi_period}, TF={rsi_interval}")
        ...
        try:
            ...
            if pinbar_candle_index is not None:
                if pinbar_candle_index < 0 or pinbar_candle_index >= len(candles):
                    logger.error(
                        f"Невалідний індекс пін-бара: {pinbar_candle_index}")
                    extremum_candle = candles[-2]
                    logger.warning(
                        f"⚠️ Використовую FALLBACK (остання закрита свічка)")
                else:
                    extremum_candle = candles[pinbar_candle_index]
                    logger.info(
                        f"✅ Використовую СИГНАЛЬНУ свічку (пін-бар) з індексом {pinbar_candle_index}")
                ...
                logger.info(
                    f"🔍 Шукаю локальний екстремум за останні 20 свічок...")

                lookback_candles = candles[-21:-
                                           1] if len(candles) >= 21 else candles[:-1]
                ...
                if direction.lower() == "long":
                    ...
                    logger.info(
                        f"✅ Знайдено МІНІМАЛЬНИЙ LOW за {len(lookback_candles)} свічок: {signal_extremum:.8f}")
                else:
            ...
            logger.info(
                f"   - ЧАС: {candle_time.strftime('%H:%M:%S %d.%m.%Y')}")
            ...
            if stop_buffer_type and stop_buffer_value is not None:
                try:
                    ...
                    if stop_buffer_type == 'percent':
                        buffer_distance = signal_extremum * \
                            (buffer_value_float / 100.0)
            ...
            if buffer_distance < min_offset_distance:
                logger.warning(
                    f"⚠️ Буфер ({buffer_distance:.8f}) менший за мінімальний ({min_offset_distance:.8f})")
                logger.warning(
                    f"⚠️ Використовую мінімальний відступ ЗА екстремум: {min_offset_distance:.8f}")

            if direction.lower() == "long":
                stop_price = self._quantize_price(
                    signal_extremum - final_offset, price_step, ROUND_DOWN)
                if stop_behind_offset > 0:
                    logger.info(
                        f"✅ LONG: SL = LOW - {stop_behind_offset*100:.2f}%")
                else:
                    logger.info(f"✅ LONG: SL = LOW (точно на екстремумі)")
            else:
                stop_price = self._quantize_price(
                    signal_extremum + final_offset, price_step, ROUND_UP)
                if stop_behind_offset > 0:
                    logger.info(
                        f"✅ SHORT: SL = HIGH + {stop_behind_offset*100:.2f}%")
                else:
            ...
            return {
                ...
                "stop_behind_offset_percent": stop_behind_offset * 100,
            ...
            logger.error(
                f" Помилка отримання екстремуму свічки для {pair}: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return None

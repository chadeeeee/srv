...
def is_pinbar(o, h, l, c, direction, tail_ratio, pinbar_size, avg_range, tail_percent_min=70, body_percent_max=20, opposite_percent_max=10):
    ...
    return (tail_to_body_ratio >= tail_ratio and 
            tail_percent >= tail_percent_min and 
            body_percent <= body_percent_max and 
            opposite_tail_percent < opposite_percent_max)
...
def detect_pinbar(candles, direction, pinbar_body_ratio=2.5):
    """
    ...
    for i in range(len(candles) - 2, max(len(candles) - 6, 0), -1):
        test_candles = candles[:i+1]
        
        if not is_extremum_candle(test_candles, direction, lookback=5):
            continue
        ...
        try:
            ...
            if direction == 'long':
                if lower_wick > body * pinbar_body_ratio and c > o:
                    return {
                        'timestamp': timestamp,
                        'open': o,
                        'high': h,
                        ...
                        'is_extremum': True,
                        'candle_index': i  # ДОДАНО: індекс пінбар свічки
                    }
            
            elif direction == 'short':
                if upper_wick > body * pinbar_body_ratio and c < o:
                    return {
                        'timestamp': timestamp,
                        'open': o,
                        'high': h,

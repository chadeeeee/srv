    ...
    async def handler_event(event):
        try:
            ...
            for line in lines:
                ...
                if "сопротивление" in lower or "short" in lower:
                    direction = "short"
                elif "поддержка" in lower or "long" in lower:
                    direction = "long"
            ...
            if pair and target_price and direction:
                ...
                # Check IGNORE_PINBAR setting
                ignore_pinbar = CONFIG.get('IGNORE_PINBAR', False)
                final_order_price = target_price
                order_type = "signal"  # "signal" or "extremum"
                
                if ignore_pinbar:
                    try:
                        # Get current candle data to place order at extremum
                        settings = get_settings()
                        tf_display = settings.get('timeframe', '1m')
                        tf_api = handler._parse_timeframe_for_api(tf_display)
                        ...
                        if candles and len(candles) >= 2:
                            ...
                            # ИСПРАВЛЕНО: Set order price based on signal direction
                            if direction == "long":
                                final_order_price = c_high  # LONG: Buy at HIGH
                                logger.info(f" IGNORE_PINBAR=True: LONG ордер буде на HIGH свічки: {c_high:.8f}")
                            else:  # short
                                final_order_price = c_low  # SHORT: Sell at LOW
                                logger.info(f" IGNORE_PINBAR=True: SHORT ордер буде на LOW свічки: {c_low:.8f}")
                            
                            order_type = "extremum"
                            
                            await notify_user(
                                f" IGNORE_PINBAR АКТИВНИЙ!\n"
                                f"━━━━━━━━━━━━━━━━━━\n"
                                f" {pair}\n"
                                f" Стратегія: {channel_strategy.name}\n"
                                f" Напрямок: {direction.upper()}\n"
                                f" Ціна сигналу: {target_price:.8f}\n"
                                f" Ціна ордера: {final_order_price:.8f}\n"
                                f" Свічка (TF {tf_display}):\n"
                                f"   High: {c_high:.8f}\n"
                                f"   Low: {c_low:.8f}\n"
                                f" Лімітка буде виставлена на {'HIGH' if direction == 'long' else 'LOW'} свічки!"
                            )
                        else:
                            logger.warning(f" Не вдалося отримати свічки для {pair}, використовую ціну сигналу")
                            await notify_user(
                                f" IGNORE_PINBAR активний, але не вдалося отримати дані свічки для {pair}\n"
                                f"Використовую ціну сигналу: {target_price:.8f}"
                            )
                    except Exception as e:
                        logger.error(f" Помилка при обробці IGNORE_PINBAR для {pair}: {e}")
                        await notify_user(
                            f" Помилка IGNORE_PINBAR для {pair}: {e}\n"
                            f"Використовую ціну сигналу: {target_price:.8f}"
                        )
                else:
                    # Normal signal processing
                    try:
                        # Отримуємо дані свічки перед розміщенням ордера
                        settings = get_settings()
                        tf_display = settings.get('timeframe', '1m')
                        tf_api = handler._parse_timeframe_for_api(tf_display)
                        ...
                        await notify_user(
                            f" СИГНАЛ З КАНАЛУ {event.chat_id}\n"
                            f"━━━━━━━━━━━━━━━━━━\n"
                            f" {pair}\n"
                            f" Напрямок: {direction.upper()}\n"
                            f" Ціна сигналу: {target_price:.8f}\n"
                ...
                try:
                    # Use final_order_price instead of target_price
                    result = await handler.place_signal_limit_order(pair, direction, final_order_price, channel_id=event.chat_id)

                    if isinstance(result, dict) and result.get("status") == "monitor_started":
                        logger.info(f"🛰 Запущен мониторинг уровня для {pair} — стратегия: {result.get('strategy')}")
                    elif result:
                        logger.info(f" Лимит виставлений для {pair}")
                        
                        # Логування лімітки з деталями свічки
                        try:
                            settings = get_settings()
                            tf_display = settings.get('timeframe', '1m')
                            tf_api = handler._parse_timeframe_for_api(tf_display)
                            ...
                            if candles and len(candles) >= 2:
                                ...
                                from utils.logger import limit_order_placed_with_candle
                                limit_order_placed_with_candle(pair, direction, final_order_price, c_high, c_low, tf_display)
                                
                                # Enhanced notification based on order type
                                if order_type == "extremum":
                                    await notify_user(
                                        f" ЛІМІТКА НА ЕКСТРЕМУМІ\n"
                                        f"━━━━━━━━━━━━━━━━━━\n"
                                        f" {pair}\n"
                                        f" Напрямок: {direction.upper()}\n"
                                        f" Ціна сигналу: {target_price:.8f}\n"
                                        f" Ціна лімітки: {final_order_price:.8f}\n"
                                        f" Розміщено на {'HIGH' if direction == 'long' else 'LOW'} свічки\n"
                                        f"⏱ Таймфрейм: {tf_display}\n"
                                        f" Свічка:\n"
                                        f"   High: {c_high:.8f}\n"
                                        f"   Low: {c_low:.8f}\n"
                                        f" IGNORE_PINBAR активний!"
                            ...
                            logger.error(f"Помилка логування лімітки: {e}")
                    else:
                        logger.error(f" Не удалось разместить лимитный ордер для {pair}")

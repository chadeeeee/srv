    ...
    try:
        ...
        if not row:
            ...
            """, (
                ...
                str(_settings_store.get("rsi_low")),
                str(_settings_store.get("rsi_high")),
                str(_settings_store.get("rsi_period")),
                str(_settings_store.get("rsi_interval")),
                str(_settings_store.get("max_retries")),
                str(_settings_store.get("max_open_trades")),
                str(_settings_store.get("stop_loss_offset")),
                str(_settings_store.get("pinbar_timeout")),
                str(_settings_store.get("limit_max_candles")),
                str(_settings_store.get("timeframe")),
                str(_settings_store.get("max_drawdown_percent")),
                str(_settings_store.get("equity_peak")),
...
def get_settings():
    with _lock:
        try:
            ...
            conn.close()
            if row:
                settings_dict = dict(zip(SETTING_COLUMNS, row))
...
def get_max_losses_in_row():
    with _lock:
        return int(_settings_store.get("max_losses_in_row", CONFIG.get("MAX_LOSSES_IN_ROW", 3)))

def get_pause_after_losses():
    with _lock:
        return int(_settings_store.get("pause_after_losses", CONFIG.get("PAUSE_AFTER_LOSSES", 10)))
...
def check_and_update_drawdown(current_equity):
    with _lock:
        ...
        max_dd_pct = float(_settings_store.get("max_drawdown_percent", CONFIG.get("MAX_DRAWDOWN_PERCENT", 0)))
        if peak <= 0:

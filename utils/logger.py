...
def trade_signal(pair, price, direction):
    trade_logger.info("Trade signal received", pair=pair,
...
def signal_from_channel(pair, price, direction, channel_id, strategy_name):
    ...
    strategy_display = strategy_name.name if hasattr(
        strategy_name, 'name') else str(strategy_name)
...
def limit_order_placed(direction, pair, price, info=""):
    trade_logger.info("Limit order placed",
                      direction=direction.upper(),
                      pair=pair,
                      price=price,
                      info=info)
...
def stop_loss_set(pair, price, percent):
    trade_logger.info("Stop loss set", pair=pair,
                      price=price, percent=f"{percent:.2f}%")


def take_profit_set(pair, price, percent):
    trade_logger.info("Take profit set", pair=pair,
                      price=price, percent=f"{percent:.2f}%")


def order_executed(pair, direction, price):
    trade_logger.info("Order executed",
                      direction=direction.upper(), pair=pair, price=price)


def position_closed_by_stop(pair, direction, close_price, stop_price, distance):
    trade_logger.warning("Position closed by stop loss",
                         pair=pair,
                         direction=direction.upper(),
...
def position_closed_manually(pair, direction, close_price, stop_price, distance):
    trade_logger.info("Position closed manually",
                      pair=pair,
                      direction=direction.upper(),
                      close_price=f"{close_price:.6f}",
                      stop_price=f"{stop_price:.6f}",
...
def pattern_detected(pair, pattern, direction):
    ...
    try:
        ...
        if now - last >= window:
            logger.info("Pattern detected", pattern=pattern,
                        pair=pair, direction=direction)
            _last_pattern_log[key] = now
        else:
            logger.debug("Pattern detected (throttled)",
                         pattern=pattern, pair=pair, direction=direction)
    except Exception:
        logger.info("Pattern detected", pattern=pattern,
                    pair=pair, direction=direction)
...
def divergence_detected(pair, div_type, base_rsi, current_rsi, base_price=None, current_price=None):
    try:
        ...
        if base_price is not None and current_price is not None:
            context.update({
                "base_price": f"{float(base_price):.6f}",
...
def rsi_auto_close(pair, direction, rsi_value, threshold):
    try:
        trade_logger.info("Auto-close by RSI",
                          pair=pair,
                          direction=direction.upper(),
                          rsi=f"{float(rsi_value):.2f}",
                          threshold=f"{float(threshold):.2f}")
    except Exception:
        trade_logger.warning("Auto-close by RSI", pair=pair,
                             direction=direction.upper())


def external_position_detected(pair, direction, size, entry_price, source="manual"):
    try:
        trade_logger.warning("External position detected",
                             pair=pair,
                             direction=direction.upper(),
...
def external_position_monitoring_started(pair, direction):
    ...
    trade_logger.info("RSI monitoring started", pair=pair,
                      direction=direction.upper())
...
def candle_high_low_info(pair, timeframe, high, low):
    ...
    try:
        logger.debug("Candle High/Low",
                     pair=pair,
                     timeframe=timeframe,
                     high=f"{float(high):.6f}",
                     low=f"{float(low):.6f}")
...
def signal_candle_detected(pair, direction, ohlc, is_extremum, extremum_type):
    ...
    try:
        logger.info("Signal candle detected",
                    pair=pair,
                    direction=direction.upper(),
                    open=f"{ohlc['open']:.8f}",
                    high=f"{ohlc['high']:.8f}",
...
def limit_order_params(pair, direction, entry_price, signal_candle_ohlc, stop_price, stop_buffer_info):
    ...
    try:
        trade_logger.info("Limit order parameters",
                          pair=pair,
                          direction=direction.upper(),
...
def stop_calculation_details(pair, direction, signal_extremum, buffer_type, buffer_value, final_stop, signal_candle_time=None):
        ...
        if signal_candle_time:
            try:
                ...
                dt = datetime.datetime.fromtimestamp(
                    int(signal_candle_time) / 1000)
                context["signal_candle_time"] = dt.strftime(
                    '%H:%M:%S %d.%m.%Y')
            except Exception:
                pass
...
def manual_position_opened(pair, direction, entry_price, stop_price, candle_high, candle_low, timeframe):
    ...
    try:
        trade_logger.info("Manual position opened",
                          pair=pair,
                          direction=direction.upper(),
                          entry_price=f"{entry_price:.8f}",
                          stop_price=f"{stop_price:.8f}",
    ...
    try:
        ...
        if entry_price:
            pnl_percent = ((close_price - entry_price) / entry_price *
                           100) if direction == "long" else ((entry_price - close_price) / entry_price * 100)
...
def limit_order_placed_with_candle(pair, direction, limit_price, candle_high, candle_low, timeframe):
    ...
    try:
        trade_logger.info("Limit order placed with candle",
                          pair=pair,
                          direction=direction.upper(),
...
def bybit_ping_info(pair, ping_ms, server_time_ms, local_time_ms):
    ...
    try:
        ...
        if ping_ms > 1000:
            logger.warning("High ping detected",
                           ping_ms=ping_ms, threshold=1000)
        elif ping_ms > 500:
            logger.warning("Elevated ping detected",
                           ping_ms=ping_ms, threshold=500)
    except Exception as e:
...
def rsi_settings_from_db(pair, rsi_low, rsi_high, rsi_period, rsi_interval):
    ...
    try:
        logger.info("RSI settings from DB",
                    pair=pair,
